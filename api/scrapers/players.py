...
async def vlr_player_matches(player_id: str, page: int = 1) -> dict:
    ...
    async def build():
        ...
        client = get_http_client()
        # VLR.gg responds in <1s when healthy. A single 10s attempt fits
        # comfortably inside Railway's ~25s gateway timeout. No retries —
        # if it fails, fail fast and let the frontend retry.
        resp = await fetch_with_retries(url, client=client, timeout=10, max_retries=1)
        status = resp.status_code
